...
*/
CREATE TABLE [Result_Table_KNN_Prediction_Cosine1](
	[ID] INT NOT NULL,
...
CREATE TABLE #Temp(
	...
	[isAttack] BIT NOT NULL,
	[attackGroup] NVARCHAR(5) NOT NULL,
	[distance] DECIMAL(38, 2) NOT NULL DEFAULT 1
       ...
       */
		...
		*/
		...
		( 1 - 
			...
			(SQRT(
				...
				SQUARE(src_bytes) +
				SQUARE(dst_bytes) +
				SQUARE(wrong_fragment) +
				SQUARE(urgent) +
				SQUARE(hot) +
				SQUARE(num_failed_logins) +
				SQUARE(num_compromised) +
				SQUARE(num_root) +
				SQUARE(num_file_creations) +
				SQUARE(num_shells) +
				SQUARE(num_access_files) +
				SQUARE(num_outbound_cmds) +
				SQUARE(count) + 
				SQUARE(srv_count) +
				SQUARE(serror_rate) +
				SQUARE(srv_serror_rate) +
				SQUARE(rerror_rate) +
				SQUARE(srv_rerror_rate) +
				SQUARE(same_srv_rate) +
				SQUARE(diff_srv_rate) +
				SQUARE(srv_diff_host_rate) +
				SQUARE(dst_host_count) +
				SQUARE(dst_host_srv_count) +
				SQUARE(dst_host_same_srv_rate) +
				SQUARE(dst_host_diff_srv_rate) +
				SQUARE(dst_host_same_src_port_rate) +
				SQUARE(dst_host_srv_diff_host_rate) +
				SQUARE(dst_host_serror_rate) +
				SQUARE(dst_host_srv_serror_rate) +
				SQUARE(dst_host_rerror_rate) +
				SQUARE(dst_host_rerror_rate) +
				SQUARE(dst_host_srv_rerror_rate)
				) * 
			SQRT(
				SQUARE(@duration) +
				SQUARE(@src_bytes) +
			...
			) -- end of second member of expression	
		) AS Distance_Range
		FROM MMN_Train_Set
		...
		DECLARE @num INT
		SET @num = 0
		SELECT @num = COUNT(ID) FROM #Temp
		
		IF @num = 0 
		BEGIN
			INSERT INTO #Temp(ID, isAttack, attackGroup)
			SELECT TOP(3) id, is_attack, attack_group
			FROM MMN_Train_Set
			WHERE (service <> @service) OR (protocol_type <> @protocol_type) OR (flag <> @flag)
		END
		ELSE IF @num = 1 
		BEGIN
			INSERT INTO #Temp(ID, isAttack, attackGroup)
			SELECT TOP(2) id, is_attack, attack_group
			FROM MMN_Train_Set
			WHERE (service <> @service) OR (protocol_type <> @protocol_type) OR (flag <> @flag)
		END
		ELSE IF @num = 2 
		BEGIN
			INSERT INTO #Temp(ID, isAttack, attackGroup)
			SELECT TOP(1) id, is_attack, attack_group
			FROM MMN_Train_Set
			WHERE (service <> @service) OR (protocol_type <> @protocol_type) OR (flag <> @flag)
		END
		...
		*/
		...
		BEGIN
			...
			INSERT INTO Result_Table_KNN_Prediction_Cosine1(ID, isAttack, isAttackPrediction, attackGroup, attackGroupPrediction)
